...
#   source("scripts/ch2/09_apply_prosail_full.R")
# ---
  ...
  for (site in sites) {
    ...
    for (h_min in h_min_values) {
      ...
      for (Samplingmethod in Samplingmethods) {
        ...
        n_combi  <- length(combination_labels)
        step_pct <- max(1L, n_combi %/% 10L)
        t_inv    <- proc.time()
        cat(sprintf("  SVR inversion: 0%%"))
        ...
        for (i in seq_along(combination_labels)) {
          ...
          if (i %% step_pct == 0L || i == n_combi) {
            elapsed <- round((proc.time() - t_inv)[["elapsed"]])
            cat(sprintf(" %d%%", round(i / n_combi * 100)))
          }
        }
        cat(sprintf(" — %d s\n", round((proc.time() - t_inv)[["elapsed"]])))
