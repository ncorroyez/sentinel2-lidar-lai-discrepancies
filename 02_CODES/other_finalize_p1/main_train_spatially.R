...
excluded_metrics <- paste0(
  ...
  "/aspect_res_10_m.tif$|",
  "/solar_incidence_angle_res_10_m.tif$|",
  "/slope_res_10_m.tif$|",
  "/s2lai_res_10_m.tif$"
)
  ...
  for (ind in i_test_zone){
    for (s2param in s2params){
      for (site in sites){
        ...
        final_train_set <- train_zones
        final_train_set <- final_train_set[sample(1:nrow(final_train_set)), ]
        final_test_set_sfs <- test_zone
        final_test_set_sfs <- final_test_set_sfs[sample(1:nrow(final_test_set_sfs)), ]
        final_test_set <- test_zone
        final_test_set <- final_test_set[sample(1:nrow(final_test_set)), ]
        ...
        # Aspect as factor
        # final_train_set$aspect <- as.factor(floor(final_train_set$aspect))
        # final_test_set_sfs$aspect <- as.factor(floor(final_test_set_sfs$aspect))
        # final_test_set$aspect <- as.factor(floor(final_test_set$aspect))
      ...
      rf_model <- glm(target ~ ., data = combined_train_set, family = gaussian)
      ...
      a
      # Setup
      sfs_tests <- list(combined_test_set, combined_test_set_sfs)
      for(sfs_test in sfs_tests){
        
        # Selection on summer only or on both seasons
        if(identical(sfs_test, combined_test_set)) {
          selection_season <- 'selection_on_summer'
        } else if(identical(sfs_test, combined_test_set_sfs)) {
          selection_season <- 'selection_on_summer_and_winter'
        } else {
          stop("Selection Test Set is wrong")
        }
        for (algo in algos){
          ...
          zone_name <- paste0("test_zone_", ind)
          
          # Define paths and parameters
          figure_path_split <- file.path('../04_FIGURES', 
                                         "ML_Figures",
                                         "FS",
                                         forest_composition,
                                         algo,
                                         study_name,
                                         s2param,
                                         paste0("Sampling_Size_", sampling_size),
                                         selection_season,
                                         zone_name
          )
          figure_path_all <- file.path('../04_FIGURES', 
                                       "ML_Figures",
                                       "FS",
                                       forest_composition,
                                       algo,
          ...
          if (!dir.exists(figure_path_split)) {
            dir.create(figure_path_split, showWarnings = FALSE, recursive = TRUE)
          }
          if (!dir.exists(figure_path_all)) {
            dir.create(figure_path_all, showWarnings = FALSE, recursive = TRUE)
          }
          
          # Prepare variables for SFS
          Vars_SFS <- names(combined_train_set)[-1]  # Exclude the target column
          
          # Perform SFS for the entire training data
          message('**Performing SFS with ', algo, ' for Test Zone n°', ind, '**')
          # Call SFS_Regression for the entire training set
          outSFS <- SFS_Regression(algo = algo,
                                   TrainingData = combined_train_set,
                                   TestData = sfs_test, # combined_test_set_sfs
                                   Vars_SFS = Vars_SFS,
                                   force_s2lai = force_s2lai,
                                   ntree = ntree,
                                   site = zone_name,
                                   figure_path_site = figure_path_split,
                                   nbWorkers = nbWorkers)
          
          # Store the results in a nested list structure
          SelectedVars_RF <- outSFS$SelectedVars
          EvolCorr_RF <- outSFS$EvolCorr
          EvolRMSE_RF <- outSFS$EvolRMSE
          print(SelectedVars_RF)
          a
          # Save results for all splits
          save_results_to_file(selected_vars = SelectedVars_RF,
                               evol_corr = EvolCorr_RF,
                               evol_rmse = EvolRMSE_RF,
                               figure_path_site = figure_path_split,
                               site = zone_name,
                               mask_name = forest_composition)
        }
      }
    }
  }
