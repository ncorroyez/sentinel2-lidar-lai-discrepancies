...
compute_weights_per_metric_rf <- function(metrics_df) {
  ...
  out <- lapply(unique(metrics_long$metric), function(met) {
    lapply(unique(metrics_long$site), function(st) {
      ...
      data_split <- initial_split(df_sub, prop = 0.8)
      ...
      shap_vals <- fastshap::explain(
        object = mod,
        X = test_data[, c("PROSAIL", "Depth", "Het")],
        pred_wrapper = pred_fun,
        nsim = 100  # increase for stability
      )
      
      # Convert to importance = mean(|SHAP|)
      imp_vals <- colMeans(abs(shap_vals), na.rm = TRUE)
      print(imp_vals)
      ...
      # Explained/residual variance (RF R²)
