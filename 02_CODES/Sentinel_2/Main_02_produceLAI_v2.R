...
for (site in sites){
  ...
  PROSAIL_ResPath <- file.path(output_dir[[site]],'HybridInversion_2')
  ...
  InputPROSAIL <- get_InputPROSAIL(atbd = TRUE, 
                                   GeomAcq = GeomAcq)
  InputPROSAIL$LIDFa <- runif(n = length(InputPROSAIL$LIDFa), 
                              min = 30, max = 50)
  ...
  BRF_LUT_Noise <- apply_noise_atbd(BRF_LUT)
  rownames(BRF_LUT_Noise) <- SRF$Spectral_Bands
  modelSVR <- list()
  modelSVR$lai <- PROSAIL_Hybrid_Train(BRF_LUT = BRF_LUT_Noise[Bands2Select$lai, ],
