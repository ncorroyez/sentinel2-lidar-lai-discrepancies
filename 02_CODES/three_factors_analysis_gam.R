...
compute_weights_per_metric_gam <- function(metrics_df) {
  library(mgcv)
  ...
  out <- lapply(unique(metrics_long$metric), function(met) {
    lapply(unique(metrics_long$site), function(st) {
      ...
      # Fit GAM (categorical for PROSAIL, Depth; smooth for Het)
      mod <- mgcv::gam(
        Y ~ PROSAIL + Depth + s(Het, k = 5),
        data = df_sub,
        method = "REML"
      )
      ...
      # Get deviance explained per term
      a <- anova(mod)
      dev_tab <- as.data.frame(a)
      dev_terms <- dev_tab$sig # approximate chi-sq or F test stat
      
      names(dev_terms) <- rownames(dev_tab)
      print(dev_terms)
      
      # Keep only predictors of interest
      imp_vals <- dev_terms[c("PROSAIL", "Depth", "s(Het)")]
...
weights  <- compute_weights_per_metric_gam(metrics)
