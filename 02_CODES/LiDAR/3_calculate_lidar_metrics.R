...
for (site in sites){
  ...
  las_l93_dir <- file.path(data_dir,
                           site,
                           "LiDAR",
                           leaf_state,
                           "1-las_l93"
  ...
  las_norm_utm_dir <- file.path(data_dir,
                                ...
                                "LiDAR",
                                leaf_state,
                                "3-las_normalized_utm" # 3-las_normalized_utm
  )
  ...
  # plot(ctg)
  # ctg_las <- readLAScatalog(las_l93_dir)
  # plot(ctg_las)
  # stop()
  # st_write(st_as_sf(ctg), file.path(data_dir,
  #                                   site,
  #                                   "LiDAR",
  #                                   leaf_state,
  #                                   "utm_from_lidar.gpkg"), 
  ...
  opt_select(normalized_ctg) <- "xyzc"
  ...
  # canopy_cover_file <- "fCover_res_25_m.tif"
  ...
  # VCI (DSM)
  # z_opt <- switch(site, "Aigoual" = 5, "Blois" = 5, "Mormal" = 9)
  # 
  # vci <- lidR::pixel_metrics(normalized_dsm_ctg, 
  #                            ~VCI_dsm(Z, zmax = 40, z_opt = z_opt), 
  #                            res = res)
  # vci <- terra::project(vci, mask_10m)
  # vci_file <- "vci_no_ground.tif"
  # vci_file <- "vci_dsm_res_10_m.tif"
  # writeRaster(vci,
  #             filename = file.path(raw_dir, vci_file),
  #             filetype = "GTiff",
  #             overwrite = TRUE)
  ...
  # pad_rasters <- lidR::pixel_metrics(normalized_ctg,
  #                                    ~myPAD_dtm_new(Z, Classification),
  #                                    # ~end_lad_dtm(Z, Classification),
  #                                    res=s2_rast)
  # # pad_rasters <- terra::project(pad_rasters, mask_10m)
  ...
  # 
  # # bonus
  # lad_filename <- "ladstack_dtm_0408.tif"
  # writeRaster(lad_raster,
  #             file.path(metrics_dir, "Deciduous_Only"),
  #             overwrite = TRUE)
  # 
  ...
  #                                    res=s2_rast)
  # # pad_rasters <- terra::project(pad_rasters, mask_10m)
  # if (!dir.exists(file.path(raw_dir, "PAD_Profiles_Classic"))) {
  #   dir.create(file.path(raw_dir, "PAD_Profiles_Classic"),
  #              showWarnings = FALSE,
  #              recursive = TRUE)
  # }
  # # Get the names of all layers
  ...
  # lad_layers <- layer_names[grepl("LAD_Layer_", layer_names)]
  # lad_extracted_values <- as.numeric(sub("LAD_Layer_", "", lad_layers))
  # lad_sorted_indices <- order(lad_extracted_values, decreasing = FALSE)
  # lad_selected_layers <- lad_layers[lad_sorted_indices]
  # lad_raster <- subset(pad_rasters, lad_selected_layers)
  # lad_filename <- "ladstack.tif"
  # writeRaster(lad_raster,
  ...
  # #                      metrics_dir)
  # 
  # if (!dir.exists(file.path(raw_dir, "PAD_Profiles_dsm_pit"))) {
  #   dir.create(file.path(raw_dir, "PAD_Profiles_dsm_pit"),
  #              showWarnings = FALSE,
  #              recursive = TRUE)
  # }
  # # Get the names of all layers
  ...
  # 
  # # bonus
  # # lad_filename <- "ladstack_dsm_0408.tif"
  # # writeRaster(lad_raster,
  # #             file.path(metrics_dir, "Deciduous_Only", lad_filename),
  # #             overwrite = TRUE)
  # 
  # # Extract and save the next 38 PAI layers individually
  # pai_layers <- layer_names[grepl("PAI_", layer_names)]
  ...
  # Max (percentiles)
  fact_val <- round(terra::res(mask_10m)[1] / terra::res(chm)[1])
  
  # 2. Define the percentiles you want to calculate
  probs <- c(0.95, 0.98)
  
  for (p in probs) {
    
    # Construct a name for the metric (e.g., "p95", "p98")
    p_name <- paste0("max_p", p * 100)
    
    # Aggregate first because 'project' does not support custom functions
    agg_rast <- terra::aggregate(chm, 
                                 fact = fact_val, 
                                 fun = function(x) quantile(x, probs = p, na.rm = TRUE))
    
    # final_rast <- terra::project(agg_rast, mask_10m, method = "near")
    out_filename <- paste0(p_name, "_res_10_m.tif")
    terra::writeRaster(final_rast,
                       filename = file.path(raw_dir, out_filename),
                       filetype = "GTiff",
                       overwrite = TRUE)
    apply_and_save_masks(final_rast,
                         out_filename,
                         masks_dir,
                         metrics_dir)
  }
  ...
  # MNS: sd and cv
  # mns_sd <- aggregate(dsm, fact = 10, fun = "sd", na.rm = TRUE)
  # mns_mean <- aggregate(dsm, fact = 10, fun = "mean", na.rm = TRUE)
  # mns_cv <- mns_sd / mns_mean
  # mns_cv <- project(mns_cv, mask_10m)
  # mns_cv <- mask(mns_cv, mask_10m)
  # mns_sd <- project(mns_sd, mask_10m)
  # mns_sd <- mask(mns_sd, mask_10m)
  # 
  # writeRaster(mns_sd,
  #             filename = file.path(raw_dir, "dsm_sd_res_10_m.tif"),
  #             filetype = "GTiff",
  #             overwrite = TRUE)
  # apply_and_save_masks(mns_sd,
  #                      "dsm_sd_res_10_m.tif",
  #                      masks_dir,
  #                      metrics_dir)
  # 
  # writeRaster(mns_cv,
  #             filename = file.path(raw_dir, "dsm_cv_res_10_m.tif"),
  #             filetype = "GTiff",
  #             overwrite = TRUE)
  # apply_and_save_masks(mns_cv,
  #                      "dsm_cv_res_10_m.tif",
  #                      masks_dir,
  #                      metrics_dir)
}
