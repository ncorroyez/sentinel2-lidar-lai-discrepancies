# ------------------------------ Code Info -------------------------------------
# title: "6c_explain_heterogeneity_stability.R"
...
for (site in sites){
  ...
  for (mask_name in names(composition_masks)) {
    ...
    for(force_s2lai in force_s2lai_bools){
      ...
      # Corr Matrix
      correlation_matrix <- cor(regression_test_data$predictors_val[, ])
      
      # Set file path for the plot
      plot_file_path <- file.path(figure_path_site[[site]], 
      ...
      segment_size <- ceiling(nrow(TrainingData) / num_splits)
      ...
      for (i in 1:num_splits) {
        ...
        end_index <- min(i * segment_size, nrow(TrainingData))
        
        # Extract the segment from the training data
        TrainingSubset <- TrainingData[start_index:end_index, ]
        # ValidationSubset <- TrainingData[-(start_index:end_index), ]
