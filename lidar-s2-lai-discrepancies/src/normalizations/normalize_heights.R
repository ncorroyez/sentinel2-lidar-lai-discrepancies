...
for (site in sites){
  ...
  st_crs(las) <- st_crs(s2_rast)
  # las <- clip_circle(las, xcenter = median(las@data$X), ycenter = median(las@data$Y),
  las <- clip_circle(las, xcenter = 371925, ycenter = 5271025,
                     radius = 20)
  # 
  thr <- 2
  # # DTM
  ...
  dsm_filled <- terra::focal(dsmm, w = matrix(1, 3, 3), fun = fill.na)
  ...
  opt_select(ctg_dtm) <- "xyzc"
  # opt_output_files(ctg_dtm) <- file.path(las_norm_dsm_dir, "{XLEFT}_{YBOTTOM}_norm")
  opt_output_files(ctg_dtm) <- file.path("/media/corroyez/My Passport/01_DATA/Mormal/LiDAR/4_dsm", "{XLEFT}_{YBOTTOM}_norm")
